...
class Board:
    def __init__(self, boardString, index) -> None:
    ...
    def __str__(self):
        ...
        return  f'Board Status: {status_text}' + f'Baord Index: {self.index}\n' + ''.join([f'{r}\n' for r in self.rows])
...
def eval_boards(boards, index):
    for call_index in range(index, CALL_COUNT):
        for b in boards:
            if(b.markNumber(calls[call_index]) and len(boards) == 1):
                print(b.calc_board()* calls[call_index])
                exit()
            elif(b.markNumber(calls[call_index])):
                boards.remove(b)
                return eval_boards(boards, call_index)

eval_boards(boards, 0)
